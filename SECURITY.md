...
This document describes the security properties of the MongrelDB Objective-C
client and how to report vulnerabilities.

## Overview

The MongrelDB Objective-C client is a library that talks to `mongreldb-server`
over HTTP using Apple's NSURLSession. The client itself holds no encryption
...
- Auth credentials (token, username, password) are validated to reject CR/LF
  ...
  injection (request splitting); the client refuses to construct a client with
  such a credential.
- NSURLSession is configured to never follow redirects, so an Authorization
...
- **WARNING - raw SQL:** The `sql:error:` method sends a raw SQL string to the
...
The MongrelDB Objective-C client's only runtime dependency is Apple Foundation
(NSURLSession, NSJSONSerialization), which is maintained by Apple and updated
through OS releases. Keep your OS patched. Report any client-side
vulnerabilities through the private vulnerability reporting flow below.
...
- the MongrelDB ObjC client version, compiler, and OS,
